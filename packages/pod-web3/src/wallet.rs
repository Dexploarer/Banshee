...
impl SolanaWallet {
    ...
    pub fn from_mnemonic(
        ...
        let keypair = Keypair::from_bytes(&seed[..32]).map_err(|e| {
            WalletError::InvalidMnemonic(format!("Failed to create keypair: {}", e))
        })?;
    ...
    pub fn generate_with_mnemonic(
        ...
        let wallet =
            Self::from_mnemonic(&mnemonic_phrase, None, rpc_endpoint, network, commitment)?;
    ...
    pub async fn transfer_sol(&self, to_address: &str, amount_sol: f64) -> Result<Signature> {
        ...
        let transfer_instruction =
            system_instruction::transfer(&self.keypair.pubkey(), &to_pubkey, amount_lamports);
    ...
    pub async fn request_airdrop(&self, amount_sol: f64) -> Result<Signature> {
        ...
        self.rpc_client
            ...
            .await
            .map_err(|e| {
                WalletError::TransactionFailed(format!("Failed to confirm airdrop: {}", e))
            })?;
    ...
    pub async fn get_recent_signatures(&self, limit: usize) -> Result<Vec<String>> {
        ...
        info!(
            "Retrieved {} recent signatures for wallet {}",
    ...
    pub async fn health_check(&self) -> Result<bool> {
        match self.rpc_client.get_health().await {
            Ok(_) => {
                info!(
                    "Solana RPC connection healthy for network: {}",
                    self.network
                );
                Ok(true)
            }
        ...
        if self.primary_wallet.is_none() {
            let wallet = if let Some(ref mnemonic) = self.config.wallet_mnemonic {
                ...
                return Err(anyhow::anyhow!(
                    "No wallet configured and auto-generation disabled"
                ));
            };
