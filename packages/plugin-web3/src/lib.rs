    ...
    pub fn new(web3_config: Web3PluginConfig) -> Self {
        Self {
            config: plugin_config!(
                ...
                "Web3 Plugin",
                "1.0.0",
                "Real Solana blockchain integration with July 2025 SDK"
            ),
            web3_config,
...
impl Plugin for Web3Plugin {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        match self.get_wallet_manager().await {
            Ok(mut manager) => {
                let wallet = manager.get_primary_wallet().await?;
    ...
    fn actions(&self) -> Vec<Box<dyn Action>> {
        vec![
            Box::new(TransferSolAction::new(self.web3_config.clone(), self.wallet_manager.clone())),
            Box::new(GetBalanceAction::new(self.web3_config.clone(), self.wallet_manager.clone())),
            Box::new(CreateWalletAction::new(self.web3_config.clone(), self.wallet_manager.clone())),
            Box::new(DeployTokenAction::new(self.web3_config.clone(), self.wallet_manager.clone())),
            Box::new(TransferTokenAction::new(self.web3_config.clone(), self.wallet_manager.clone())),
        ]
    }

    fn providers(&self) -> Vec<Box<dyn Provider>> {
        vec![
            Box::new(WalletInfoProvider::new(self.web3_config.clone(), self.wallet_manager.clone())),
    ...
    async fn health_check(&self) -> Result<bool> {
        match self.get_wallet_manager().await {
            Ok(mut manager) => {
                let wallet = manager.get_primary_wallet().await?;
                wallet.health_check().await.map_err(|e| e.into())
            }
            Err(e) => {
