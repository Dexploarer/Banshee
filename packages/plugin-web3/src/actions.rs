...
use crate::{Web3PluginConfig, WalletManager, wallet::utils};
...
impl Action for TransferSolAction {
    ...
    fn config(&self) -> &ActionConfig {
        ...
        CONFIG.get_or_init(|| ActionConfig {
            ...
            description: "Transfer SOL between Solana wallets with full transaction confirmation".to_string(),
            ...
            Ok(signature) => {
                ...
                Ok(ActionResult {
                    ...
                    metadata: {
                        ...
                        meta.insert("transaction_type".to_string(), serde_json::json!("sol_transfer"));
                        meta.insert("network".to_string(), serde_json::json!(self.config.network));
            ...
            Ok((wallet, mnemonic)) => {
                ...
                if request_airdrop && self.config.network == "devnet" {
                    match wallet.request_airdrop(1.0).await {
                        Ok(signature) => {
                            let new_balance = wallet.get_balance().await.unwrap_or(0.0);
                            result_data["airdrop_signature"] = serde_json::json!(signature.to_string());
                ...
                Ok(ActionResult {
                    ...
                    metadata: {
                        let mut meta = HashMap::new();
                        meta.insert("action_type".to_string(), serde_json::json!("wallet_creation"));
                        meta.insert("network".to_string(), serde_json::json!(self.config.network));
                        meta.insert("sdk_version".to_string(), serde_json::json!("2.3.1"));
                        meta
                    },
